...
* Preserve whisper.cpp's clean per-channel segment text for split stereo transcription and filter control tokens from fallback token output
* Group split-channel sentence chunks by detected WAV speech regions before merging the stereo conversation timeline
